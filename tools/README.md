...
- `test-join.sh` — force-rejoin cycle: remove Z2M's cached entry, factory-reset the device, watch it re-interview, PASS/FAIL on the result.
  ...
  `genPowerCfg` joined the list when battery reporting landed (#8). A cluster-list change also needs the Z2M-side external converter updated in step with it — see [`external-converters/xiao-power-meter.js`](../external-converters/xiao-power-meter.js).
